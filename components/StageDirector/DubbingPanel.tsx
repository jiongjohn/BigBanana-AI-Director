...
// MiMo TTS 仅 voiceclone 变体支持音频样本输入；非克隆变体（含 default / voicedesign）仍走 voice 字符串。
const isVoiceCloneModel = (model: AudioModelDefinition | undefined): boolean => {
  if (!model) return false;
  if (model.params.apiFormat !== 'mimo_tts') return false;
  const ident = (model.apiModel || model.id || '').toLowerCase();
  return ident.includes('voiceclone');
};
...
const DubbingPanel: React.FC<DubbingPanelProps> = ({ shot, onGenerateDubbing, onClearDubbing }) => {
  ...
  const voiceLibrary = project?.voiceLibrary || [];
  ...
  const [selectedVoiceSampleId, setSelectedVoiceSampleId] = useState<string>(shot.dubbing?.voiceSampleId || '');
  ...
  const needsVoiceSample = isVoiceCloneModel(resolvedDubbingModel);
  ...
  const canGenerateDubbing =
    ...
    !isGeneratingDubbing &&
    (!needsVoiceSample || !!selectedVoiceSampleId);
  ...
  useEffect(() => {
    ...
    setSelectedVoiceSampleId(shot.dubbing?.voiceSampleId || '');
  ...
  const handleGenerateDubbing = () => {
    ...
    onGenerateDubbing(
      ...
      dubbingText.trim(),
      selectedAudioModelId,
      needsVoiceSample ? selectedVoiceSampleId : undefined
    );
  };
      ...
      {needsVoiceSample && (
        <div className="space-y-2">
          ...
          {voiceLibrary.length === 0 ? (
            <div className="rounded-lg border border-dashed border-[var(--border-primary)] px-3 py-2 text-[10px] text-[var(--text-muted)]">
              当前项目音色库为空，请先到“资产库 → 音色库”上传 mp3 / wav 样本（≤ 10MB）。
            </div>
          ) : (
            <>
              <select
              ...
              >
                ...
                {voiceLibrary.map((sample) => (
                  <option key={sample.id} value={sample.id}>
                    {sample.name}
                    {sample.durationSec ? ` (${sample.durationSec.toFixed(1)}s)` : ''}
                    {sample.language ? ` · ${sample.language}` : ''}
                  </option>
                ))}
              </select>
              <p className="text-[9px] text-[var(--text-muted)]">
                选定样本将作为 MiMo VoiceClone 的克隆源；切换非克隆模型可关闭此项。
              </p>
            </>
          )}
        </div>
